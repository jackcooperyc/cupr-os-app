...
import type { IntegrationItem, IntegrationsPayload } from "@/lib/mockApi";
import { CheckCircle2, AlertCircle, RefreshCw, XCircle, Sparkles, Building2, Blocks } from "lucide-react";
...
export default function IntegrationsPage() {
  const { data, loading, error } = useCuprosMock<IntegrationsPayload>("integrations");
  const [activeCategory, setActiveCategory] = useState("All");
  const [selectedIntegration, setSelectedIntegration] = useState<IntegrationItem | null>(null);
  const [selectedConnectIntegration, setSelectedConnectIntegration] = useState<IntegrationItem | null>(null);
    ...
    return (
      <div className="max-w-7xl mx-auto pb-10">
        <EmptyState
          ...
          description={error ?? "Could not load integration catalog."}
          action={
  ...
  const handleActionClick = (integration: IntegrationItem) => {
        ...
        badges={
          <>
            ...
            <Badge variant="outline" className="bg-cupros-surface border-cupros-border text-cupros-text-muted">
              <span className="w-2 h-2 rounded-full bg-cupros-green mr-1.5" />
              {data.summary.connected} Connected
            </Badge>
            {data.summary.needsAttention > 0 && (
              <Badge variant="outline" className="bg-cupros-surface border-cupros-border text-cupros-text-muted">
                <span className="w-2 h-2 rounded-full bg-amber-500 mr-1.5" />
                {data.summary.needsAttention} Needs Attention
              </Badge>
            )}
          </>
        }
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {filteredIntegrations.length === 0 ? (
          <EmptyState
            ...
            description="Try another filter or browse all available connectors."
          />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-6">
            {filteredIntegrations.map((integration) => (
              <IntegrationCard key={integration.name} data={integration} onAction={() => handleActionClick(integration)} />
            ))}
          </div>
        )}
      </div>
...
function IntegrationCard({ data, onAction }: { data: IntegrationItem; onAction?: () => void }) {
  const getBadge = (status: IntegrationItem['status']) => {
    switch (status) {
      case "Connected":
